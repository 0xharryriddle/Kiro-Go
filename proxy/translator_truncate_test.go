...
func TestClaudeToKiroTruncatesOversizedHistory(t *testing.T) {
	...
	byteLimit := maxPayloadBytesForModel(req.Model)
	if len(raw) > byteLimit {
		t.Fatalf("payload size %d exceeds limit %d after truncation", len(raw), byteLimit)
	}
	// Truncation must still have DONE something: the fixture is ~3.4MB of
	// history, so a ceiling that silently stopped being enforced would also
	// satisfy the check above.
	if len(raw) >= 3_400_000 {
		t.Fatalf("payload %d bytes looks untruncated — byte ceiling not enforced", len(raw))
	}
