...
func (h *Handler) handleAdminBotStats(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve an apiKey filter to an entry ID up front. Keys are hashed at rest,
	// so ListApiKeys entries always carry an EMPTY Key field — comparing the
	// request's cleartext against e.Key could never match and silently returned
	// zero rows for a valid key. FindApiKeyByValue does the constant-time hash
	// comparison. An unknown key resolves to a sentinel that matches nothing, so
	// the filter still returns empty rather than degrading to "no filter".
	filterID := req.ID
	if req.ApiKey != "" {
		if entry := config.FindApiKeyByValue(req.ApiKey); entry != nil {
			if filterID != "" && filterID != entry.ID {
				// Contradictory id + apiKey: no entry can satisfy both.
				filterID = "\x00no-match"
			} else {
				filterID = entry.ID
			}
		} else {
