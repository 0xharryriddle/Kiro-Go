...
// must never be echoed to clients (json:"-" on ResponsesObject.OwnerApiKeyID).
//
// MERGE POLICY NOTE (fork ↔ upstream v1.1.5): upstream called this field
// APIKeyID and the fork calls it OwnerApiKeyID; the merged ResponsesObject keeps
// the fork's name (loadResponse still reads upstream's persisted `api_key_id` as
// a fallback so old records keep their owner). The assertion is unchanged.
func TestResponsesStorePersistsOwner(t *testing.T) {
	...
	if loaded.OwnerApiKeyID != "key_owner_abc" {
		t.Fatalf("owner not persisted: got %q want key_owner_abc", loaded.OwnerApiKeyID)
	}
