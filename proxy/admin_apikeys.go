...
type apiKeyView struct {
	...
	// MERGE POLICY NOTE (fork ↔ upstream v1.1.5): both sides added per-key rate
	// limits, and they are DIFFERENT mechanisms that happen to share a name:
	//
	//   - RpmLimit/TpmLimit (int64, fork) are HARD limits — exceeding either
	//     rejects with 429 + Retry-After (see rateLimiter.Admit in auth.go);
	//   - RPMLimit/TPMLimit (int, upstream) are a token-bucket THROTTLE that
	//     delays rather than rejects (rpmThrottle.reserve), plus display-only TPM.
	//
	// config.ApiKeyEntry already resolved the collision by tagging the hard pair
	// `rpmLimitHard`/`tpmLimitHard` (config.go:442) and leaving `rpmLimit`/
	// `tpmLimit` to the throttle pair; apiKeyUpdateRequest follows the same
	// convention. This view had BOTH pairs claiming `rpmLimit`/`tpmLimit`, which
	// vet flags and encoding/json resolves by emitting NEITHER field — so the
	// admin UI read undefined for both. The tags are aligned with config here.
...
type apiKeyCreateRequest struct {
	...
	RpmLimit    int64   `json:"rpmLimitHard,omitempty"`
	TpmLimit    int64   `json:"tpmLimitHard,omitempty"`
	// MERGE NOTE (fork ↔ upstream): ours' Rpm/TpmLimit (int64, hard 429 reject,
	// proxy/rate_limiter.go) and upstream's RPM/TPMLimit (int, soft delay,
	// proxy/rpm_limiter.go) are DIFFERENT limiters that both shipped under the
	// json tag "rpmLimit"/"tpmLimit". Two Go fields sharing one tag makes
	// encoding/json drop both, so the hard-limit pair is exposed as
	// rpmLimitHard/tpmLimitHard. Go field names are unchanged.
	ExpiresAt   int64    `json:"expiresAt,omitempty"`
	RPMLimit    int      `json:"rpmLimit,omitempty"`
	IPLimit     int      `json:"ipLimit,omitempty"`
	IPAllowlist []string `json:"ipAllowlist,omitempty"`
...
func toApiKeyExportView(e config.ApiKeyEntry) apiKeyExportView {
	...
	return apiKeyExportView{
		ID:   e.ID,
		Name: e.Name,
		// MaskApiKey(e.Key) would return "" for every stored entry: keys are held
		// HASHED at rest (KeyHash + display-only KeyMask, plaintext cleared — see
		// config.AddApiKeys), so e.Key is empty on read. ApiKeyDisplayMask reads
		// the stored KeyMask and only falls back to masking plaintext for a legacy
		// entry that has not been migrated yet. The panel view (toApiKeyView)
		// already used it; this export row was the one site still masking
		// plaintext, which silently exported a blank keyMasked column.
		KeyMasked:         config.ApiKeyDisplayMask(e),
		Enabled:           e.Enabled,
		RequestsCount:     e.RequestsCount,
		TokensUsed:        e.TokensUsed,
