...
func seedExportKeys(t *testing.T) (unlimited, under, overTok, expired string) {
	...
	// MERGE NOTE: RecordApiKeyUsage carries a 4th `model` arg in this fork (per-model
	// usage breakdown). These export assertions do not exercise model attribution, so
	// "" is passed and the usage lands under "unknown".
