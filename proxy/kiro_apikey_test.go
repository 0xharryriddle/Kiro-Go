...
func TestApiImportCredentialsApiKey(t *testing.T) {
	...
	// The import path stores the key ONCE (KiroApiKey) and leaves AccessToken
	// empty — config.UpstreamBearerToken/HasUpstreamCredential are key-aware, so
	// the legacy AccessToken mirror would only duplicate a long-lived secret at
	// rest. See TestImportKiroAPIKeyCredentialLiveValidatesAndPersists.
	if !stored.IsApiKeyCredential() || stored.KiroApiKey != "ksk_restore" || stored.ExpiresAt != 0 || stored.Region != "eu-west-1" {
		t.Fatalf("imported account not normalized: %+v", stored)
	}
	if stored.AccessToken != "" {
		t.Fatalf("import must not duplicate the key into AccessToken: %+v", stored)
	}
