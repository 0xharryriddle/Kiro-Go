...
func newCustomApiTestHandler(t *testing.T, acc config.Account) *Handler {
	...
	p.Reload()
	// UpdateStats persists account stats from a detached goroutine (kept off the
	// request path on purpose). Drain it before the harness removes the temp
	// config dir, otherwise that late config.Save() re-creates config.json /
	// config.json.bak mid-RemoveAll and the cleanup fails with "directory not
	// empty". Registered AFTER t.TempDir() above so it runs BEFORE the removal
	// (t.Cleanup is LIFO).
