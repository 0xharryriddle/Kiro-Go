...
func seedKey(t *testing.T, name string, creditLimit, creditsUsed float64) config.ApiKeyEntry {
	...
	if creditsUsed > 0 {
		// Seeding helper: no specific model, so the usage is attributed to
		// "unknown" by RecordApiKeyUsage's per-model breakdown.
	...
	// Keys are hashed at rest: the stored entry has Key cleared, and AddApiKey's
	// return value is the ONLY place the cleartext ever exists. Carry it onto the
	// refreshed copy so callers can still authenticate with entry.Key, otherwise
	// every request built from this helper sends an empty key and gets a 401.
	out := *updated
	out.Key = entry.Key
	return out
}
