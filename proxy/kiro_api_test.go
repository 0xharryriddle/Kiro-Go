...
func TestKiroProfileRegionCandidatesExternalIncludesBothDataPlanes(t *testing.T) {
	...
	// MERGE POLICY NOTE (fork ↔ upstream v1.1.5): upstream asserted here that an
	// authMethod=="idc" account stays single-region. That contract is superseded and
	// the assertion was removed rather than kept, because the two sides disagree and
	// only one can hold:
	//
	//   - upstream's premise: idc carries an authoritative profile region.
	//   - the fork's premise: idc carries the SSO PORTAL region, which is
	//     independent of where the tenant's CodeWhisperer profile lives.
	//
	// The fork's premise wins on a hard-failure argument: it was pinned by an
	// OBSERVED regression (portal ssoins-*.us-east-1 while the tenant's only
	// profile was in eu-central-1), where restricting the probe to the portal
	// region returned an empty list and the account failed outright with "no
	// available Kiro profile". Upstream's variant merely saves one probe call.
	//
	// The idc contract is now owned by, in proxy/kiro_region_test.go:
	//   - TestKiroProfileRegionCandidatesIdCProbesFallbackRegions
	//   - TestKiroProfileRegionCandidatesEnterpriseIdC
	//   - TestKiroProfileRegionCandidatesSingleRegionAuthMethods (social/""/BuilderId
	//     stay single-region; idc deliberately excluded)
	// Builder ID remains the one idc provider pinned to a single region, since
	// profile listing is unsupported for it everywhere.
	social := &config.Account{AuthMethod: "social", Region: "ap-southeast-2"}
	got = kiroProfileRegionCandidates(social)
...
func TestResolveProfileArnProbesSecondaryRegion(t *testing.T) {
	...
	account := config.Account{
		...
		Email:       "user@example.com",
		AccessToken: "access-token",
		AuthMethod:  "external_idp", // Azure-tenant login: defaults to us-east-1 auth region
		Region:      "us-east-1",    // auth/SSO region — profile lives in eu-central-1
	}
	if err := config.AddAccount(account); err != nil {
		t.Fatalf("add account: %v", err)
	}
