...
(() => {
  ...
  function renderLogs(logs) {
    ...
    for (const l of filtered) {
      ...
      const cacheWrite = l.cacheWriteTokens || 0;
      let cacheCell = '';
      if (cacheRead > 0 || cacheWrite > 0) {
        // Show the absolute token counts, not only the percentage: "how many
        // tokens did we actually serve from cache" is the number an operator
        // needs, and a share alone cannot answer it. Read is prefixed with the
        // reload glyph, write with a plus, so a cache still being populated
        // (0 reads, N writes) reads differently from a cache that is paying off
        // -- and from caching not working at all, which is what a read-only
        // display collapses those two states into.
        const share = cacheRead > 0 && l.tokens > 0 ? Math.round((cacheRead / l.tokens) * 100) : 0;
        const bits = [];
        const title = [];
        if (cacheRead > 0) {
          bits.push('\u21BB' + formatNum(cacheRead) + (share > 0 ? ' (' + share + '%)' : ''));
          title.push(t('logs.cacheReadTokens') + ': ' + formatNum(cacheRead));
        }
        if (cacheWrite > 0) {
          bits.push('+' + formatNum(cacheWrite));
          title.push(t('logs.cacheWriteTokens') + ': ' + formatNum(cacheWrite));
        }
        cacheCell = ' <span class="log-cache" title="' + escapeAttr(title.join(' / ')) + '">' +
          bits.join(' ') + '</span>';
      }
  ...
  function renderTraceDetail(d) {
    ...
    parts.push(traceField(t('logs.cacheWriteTokens'), rec.cacheWriteTokens));
  ...
  function credentialImportPayloadFromFullAccount(a) {
    ...
    // MERGE POLICY NOTE (fork <-> upstream v1.1.5): union. Upstream's helper
    // contributed the external_idp round-trip fields above; these last two come
    // from the fork's inline copy of this payload builder and must not be lost:
    //
    //   - kiroApiKey IS the credential for an api_key account, so omitting it
    //     produced an export that could never be re-imported.
    //   - an api_key account routes on regionOverride (the region its key was
    //     proven to serve). Exporting the display `region` instead could pin the
    //     re-imported account to a region the key does not serve, which 403s
    //     forever because an api_key account never re-probes.
    if (a.kiroApiKey) payload.kiroApiKey = a.kiroApiKey;
    var exportRegion = a.authMethod === 'api_key' ? (a.regionOverride || a.region) : a.region;
    if (exportRegion) payload.region = exportRegion;
    return payload;
  }
  ...
  async function loadThinkingConfig() {
    ...
    if ($('showPlaceholderReasoning')) $('showPlaceholderReasoning').checked = !!d.showPlaceholderReasoning;
  }
  async function saveThinkingConfig() {
    const res = await api('/thinking', {
      method: 'POST', body: JSON.stringify({
        ...
        showPlaceholderReasoning: $('showPlaceholderReasoning') ? $('showPlaceholderReasoning').checked : false
      })
    });
    const d = await res.json();
  ...
  async function loadPromptFilter() {
    ...
    if ($('filterPII')) $('filterPII').checked = !!d.filterPII;
    promptRules = d.rules || [];
    renderPromptRules();
  }
  async function savePromptFilter() {
    const res = await api('/prompt-filter', {
      method: 'POST', body: JSON.stringify({
        ...
        filterPII: $('filterPII') ? $('filterPII').checked : false,
        rules: promptRules
      })
    });
    const d = await res.json();
  ...
  async function importCredentials() {
    ...
    try {
      ...
      // MERGE POLICY NOTE (fork <-> upstream v1.1.5): upstream's
      // normalizeCredentialRecord wins. It maps the same fields the fork's
      // inline copy did, but reads them through one `credentials`-then-root
      // accessor, so a record that carries a key with an empty value no longer
      // falls through to the root copy inconsistently. It is also shared with
      // parseLineCredentials, so the two entry points cannot drift.
    ...
    for (const item of items) {
      ...
      const apiKeyValue = String(item.kiroApiKey || '').trim();
      // MERGE POLICY NOTE (fork <-> upstream v1.1.5): upstream's classification
      // wins. It recognises the alias on the PROVIDER field as well as the
      // method field, and treats a bare "ksk_"-prefixed access token with no
      // refresh token as an api_key record -- both shapes the fork's inline
      // version mis-classified as social, which then failed the refresh. The
      // fork's extra method aliases (kiro_api_key/kiroapikey) are folded in.
      const isApiKey = Boolean(apiKeyValue) ||
        ['api_key', 'apikey', 'kiro_api_key', 'kiroapikey'].includes(methodKey) ||
        (!item.refreshToken && String(item.accessToken || '').trim().startsWith('ksk_'));
      if (!isApiKey && !item.refreshToken) { fail++; continue; }
      if (isApiKey) {
        const key = apiKeyValue || String(item.accessToken || '').trim();
        ...
        const payload = {
          ...
          kiroApiKey: key,
          authMethod: 'api_key',
          provider: rawProvider || 'KiroAPIKey',
          // Region is deliberately NOT defaulted (upstream sent us-east-1): an
          // api_key account never re-probes after creation, so a guessed region
          // is permanent and every upstream call 403s forever. Sending it empty
          // lets the server discover the region the key actually serves, which
          // is the same decision normalizeRawCredential makes server-side.
          region: item.region || ''
        };
        try {
          const res = await api('/auth/credentials', { method: 'POST', body: JSON.stringify(payload) });
      ...
      // kiroApiKey is intentionally absent from this payload: api_key records
      // returned above, and the server rejects an OAuth import that carries key
      // material ("kiroApiKey requires authMethod=api_key").
      const payload = {
        id: item.id || '',
        email: item.email || '',
        userId: item.userId || '',
